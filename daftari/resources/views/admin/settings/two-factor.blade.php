...
<div class="max-w-2xl bg-white rounded-xl border border-slate-100 p-6">
    @if ($enabled)
        ...
        <div class="mt-6 pt-6 border-t border-slate-100">
            ...
            <form method="POST" action="{{ route('admin.settings.two-factor.recovery-codes') }}" class="max-w-sm space-y-3">
                @csrf
        ...
        <div class="mt-6 pt-6 border-t border-slate-100">
            ...
            <form method="POST" action="{{ route('admin.settings.two-factor.disable') }}" class="max-w-sm space-y-3" onsubmit="return confirm('{{ __('Turn off two-factor authentication?') }}')">
                @csrf
                <input type="password" name="current_password" required placeholder="{{ __('Current password') }}" class="w-full rounded-lg border border-slate-200 focus:border-brand-500 focus:ring-brand-500">
    ...
    @else
        ...
        <p class="text-sm text-slate-500 mt-1">{{ __('Required for platform admin accounts before you can use the rest of the admin panel. Scan this QR code with an authenticator app (Google Authenticator, Microsoft Authenticator, Authy, ...), then enter the 6-digit code it shows to confirm.') }}</p>
        ...
        <form method="POST" action="{{ route('admin.settings.two-factor.confirm') }}" class="mt-6 max-w-xs space-y-3">
            @csrf
