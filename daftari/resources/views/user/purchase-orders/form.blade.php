    ...
    <div class="flex items-center gap-3">
        ...
        <div class="relative">
            ...
            <div id="save-menu" class="hidden absolute end-0 mt-1 w-48 rounded-lg border border-slate-200 bg-white shadow-lg z-10">
                <button type="submit" form="po-form" name="post_immediately" value="1" class="block w-full text-start px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">{{ __('Save & approve') }}</button>
            </div>
        </div>
    </div>
</div>
...
<form method="POST" action="{{ route('app.purchase-orders.store') }}" id="po-form" class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6 flex items-center justify-between gap-6">
        ...
        <div class="text-end">
            <p class="text-xs font-semibold uppercase text-slate-400">{{ __('PO number') }}</p>
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div>
            ...
            <input type="date" name="order_date" id="pv-order-date" value="{{ old('order_date', optional($order->order_date)->format('Y-m-d')) }}" required class="mt-1 w-full rounded-lg border border-slate-200 focus:border-brand-500 focus:ring-brand-500">
        </div>
        <div>
...
<dialog id="preview-modal" class="rounded-2xl border border-slate-100 p-0 w-full max-w-2xl backdrop:bg-slate-900/40">
    <div class="p-6">
        <div class="flex items-start justify-between mb-4">
            <h3 class="text-lg font-bold text-slate-900">{{ __('Purchase order preview') }}</h3>
        ...
        <p class="mt-4 text-sm text-slate-500">{{ __('Supplier') }}: <span id="preview-supplier" class="font-medium text-slate-800"></span></p>
...
document.getElementById('preview-btn').addEventListener('click', () => {
    ...
    document.getElementById('preview-date').textContent = document.getElementById('pv-order-date').value || '';
