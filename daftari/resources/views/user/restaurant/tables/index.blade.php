    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        @foreach ($tables as $table)
            ...
            <div class="rounded-xl border p-4 {{ $statusStyle }}">
                ...
                @if ($openOrder)
                    <a href="{{ route('app.restaurant.orders.show', $openOrder) }}" class="mt-3 block w-full rounded-lg bg-white px-3 py-2 text-center text-xs font-semibold text-slate-700 border border-slate-200 hover:border-brand-400">{{ __('Open order :number', ['number' => $openOrder->order_number]) }}</a>
                @elseif ($table->status === 'available')
                    <form method="POST" action="{{ route('app.restaurant.orders.store') }}" class="mt-3">
                        @csrf
                        <input type="hidden" name="order_type" value="dine_in">
                        <input type="hidden" name="table_id" value="{{ $table->id }}">
                        <button type="submit" class="w-full rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-700">{{ __('Start order') }}</button>
                    </form>
                @endif
