...
<form method="POST" action="{{ $item->exists ? route('app.items.update', $item) : route('app.items.store') }}" class="max-w-2xl bg-white rounded-xl border border-slate-100 p-6 space-y-5">
    ...
    <div class="grid sm:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" max="100" name="vat_rate" value="{{ old('vat_rate', $item->vat_rate ?? 15) }}" required class="mt-1 w-full rounded-lg border border-slate-200 focus:border-brand-500 focus:ring-brand-500">
        </div>
        <div>
