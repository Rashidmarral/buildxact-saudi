...
<div class="bg-white rounded-xl border border-slate-100 p-6">
    ...
    <p class="text-sm text-slate-500 mt-1 mb-4">{{ __('Choose an eligible invoice first, and we will load its details into the credit note form.') }}</p>
...
(function () {
    ...
    function render(invoices) {
        ...
        results.innerHTML = invoices.map(inv => `
            <a href="{{ route('app.credit-notes.create') }}?invoice_id=${inv.id}" class="block px-4 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50">
                ...
                <div class="flex items-center justify-between text-xs text-slate-400 mt-1">
                    ...
                    <span>${@json(__('Creditable'))}: SAR ${inv.creditable}</span>
                </div>
            </a>
        `).join('');
        results.classList.remove('hidden');
    ...
    function search(q) {
        fetch(`{{ route('app.credit-notes.eligible-invoices') }}?q=${encodeURIComponent(q)}`)
            .then(r => r.json())
            .then(render);
    }
