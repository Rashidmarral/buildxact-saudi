...
@section('content')
<div x-data="{ tab: 'company' }" class="max-w-4xl">

    <div class="flex items-start justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold text-slate-900">{{ __('Settings') }}</h2>
            <p class="text-sm text-slate-500 mt-1">{{ __('Company profile, security, integrations, and business rules.') }}</p>
        </div>
    </div>
    ...
    <div x-show="tab === 'company'" x-cloak>
        <form method="POST" action="{{ route('app.settings.update') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-slate-100 p-6 space-y-5">
            @csrf
            @method('PUT')
        ...
        <div class="mt-6 bg-white rounded-xl border border-slate-100 p-6 flex items-center justify-between">
            <div>
    ...
    {{-- ============ Integrations ============ --}}
    <div x-show="tab === 'integrations'" x-cloak class="grid sm:grid-cols-2 gap-4">
        @foreach ([
            ['title' => __('API tokens'), 'desc' => __('Generate personal access tokens to use the Daftari API from your own scripts and integrations.'), 'route' => 'app.settings.api-tokens'],
            ['title' => __('Webhooks'), 'desc' => __('Notify your own systems when invoices are created, sent, or paid.'), 'route' => 'app.settings.webhooks.index'],
            ['title' => __('Payment gateways'), 'desc' => __('Let clients pay your invoices online with mada, cards, Apple Pay, or STC Pay.'), 'route' => 'app.settings.payment-gateways'],
            ['title' => __('WhatsApp'), 'desc' => __('Send invoices to clients over WhatsApp using your own WhatsApp Business account.'), 'route' => 'app.settings.whatsapp'],
            ['title' => __('SMS'), 'desc' => __('Send invoices and reminders to clients over SMS using your own Unifonic account.'), 'route' => 'app.settings.sms'],
        ] as $card)
            <div class="bg-white rounded-xl border border-slate-100 p-6 flex flex-col justify-between">
                <div>
                    <h3 class="font-semibold text-slate-900">{{ $card['title'] }}</h3>
                    <p class="text-sm text-slate-500 mt-1">{{ $card['desc'] }}</p>
                </div>
                <a href="{{ route($card['route']) }}" class="mt-4 inline-flex self-start rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-300">{{ __('Manage') }}</a>
            </div>
        @endforeach
    </div>
