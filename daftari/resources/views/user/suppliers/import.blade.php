...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-900">{{ __('Import suppliers') }}</h1>
    <p class="text-sm text-slate-500 mt-1">{{ __('Add many suppliers at once from a CSV file.') }}</p>
</div>

@include('partials.csv-import-form', [
    'importAction' => route('app.suppliers.import.store'),
    'templateUrl' => route('app.suppliers.import.template'),
    'backUrl' => route('app.suppliers.index'),
    'columns' => [
