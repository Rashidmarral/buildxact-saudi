...
<div class="flex items-center justify-between mb-6 print:hidden">
    ...
    <div class="flex items-center gap-3">
        @if ($voucher->status === 'issued')
            <a href="{{ route('app.receipt-vouchers.edit', $voucher) }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-300">{{ __('Edit') }}</a>
            <form method="POST" action="{{ route('app.receipt-vouchers.void', $voucher) }}" onsubmit="return confirm('{{ __('Void this receipt voucher? This reverses any linked invoice payment.') }}')">
                @csrf
        ...
        @else
            <form method="POST" action="{{ route('app.receipt-vouchers.destroy', $voucher) }}" onsubmit="return confirm('{{ __('Permanently delete this voided voucher? This cannot be undone.') }}')">
                @csrf
                @method('DELETE')
        ...
        @endif
        <a href="{{ route('app.receipt-vouchers.pdf', $voucher) }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-300">{{ __('Download PDF') }}</a>
