...
@section('title', isset($voucher) ? __('Edit Receipt Voucher') : __('New Receipt Voucher'))

@section('content')
<div class="mb-2">
    <h2 class="text-lg font-semibold text-slate-900">{{ isset($voucher) ? __('Edit Receipt Voucher') : __('New Receipt Voucher') }}</h2>
    <p class="text-sm text-slate-500 mt-1">{{ __('Create a clear voucher with the amount, party, and payment details in one place.') }}</p>
</div>

<form method="POST" action="{{ isset($voucher) ? route('app.receipt-vouchers.update', $voucher) : route('app.receipt-vouchers.store') }}">
    @csrf
    @if (isset($voucher))
        @method('PUT')
    @endif
    ...
    <div class="flex items-center justify-between my-6">
        <a href="{{ isset($voucher) ? route('app.receipt-vouchers.show', $voucher) : route('app.receipt-vouchers.index') }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-300">&larr; {{ __('Back') }}</a>
        ...
        <div class="bg-white rounded-xl border border-slate-100 p-6">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="payer_name" id="payer_name" value="{{ old('payer_name', $voucher->payer_name ?? '') }}" required class="mt-1 w-full rounded-lg border border-slate-200 focus:border-brand-500 focus:ring-brand-500">
                </div>
                <div>
...
(function () {
    ...
    const initialInvoiceId = {{ old('invoice_id', $voucher->invoice_id ?? '') ? (int) old('invoice_id', $voucher->invoice_id) : 'null' }};
    const currentInvoice = @json($currentInvoice ?? null);
    ...
    function addInvoiceOption(invoice) {
        if (invoicesById[invoice.id]) return;
        invoicesById[invoice.id] = invoice;
        const opt = document.createElement('option');
        opt.value = invoice.id;
        opt.textContent = invoice.invoice_number + ' — SAR ' + invoice.balance;
        invoiceSelect.appendChild(opt);
    }

    function loadInvoices(preselectId) {
        ...
        fetch('{{ url('/app/clients') }}/' + clientSelect.value + '/outstanding-invoices')
            .then(r => r.json())
            .then(invoices => {
                invoices.forEach(addInvoiceOption);
                // The invoice currently linked to this voucher may already
                // be fully settled by this very payment and so be missing
                // from the outstanding list — keep it selectable regardless.
                if (currentInvoice && preselectId && String(currentInvoice.id) === String(preselectId)) {
                    addInvoiceOption(currentInvoice);
                }
                if (preselectId && invoicesById[preselectId]) {
                    invoiceSelect.value = preselectId;
    ...
    if (clientSelect.value) loadInvoices(initialInvoiceId);
})();
</script>
@endsection
