        ...
        <div class="rounded-xl border border-slate-100 bg-white p-6 space-y-3">
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" id="rv-number" value="RV-0001" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
                </div>
                <div>
                    <label class="block text-xs text-slate-500 mb-1">{{ __('Date') }}</label>
                    <input type="date" id="rv-date" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
                </div>
            </div>
            <div>
                <label class="block text-xs text-slate-500 mb-1">{{ __('Received from') }}</label>
                <input type="text" id="rv-from" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
            </div>
            <div>
                <label class="block text-xs text-slate-500 mb-1">{{ __('Amount (SAR)') }}</label>
                <input type="number" id="rv-amount" min="0" step="0.01" value="0" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
            </div>
            <div>
                <label class="block text-xs text-slate-500 mb-1">{{ __('Payment method') }}</label>
                <select id="rv-method" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
            ...
            <div>
                ...
                <input type="text" id="rv-reason" placeholder="{{ __('e.g. invoice #, deposit, service') }}" class="w-full rounded-lg border-slate-200 text-sm focus:border-brand-500 focus:ring-brand-500">
            </div>
        </div>
