        ...
        <div class="space-y-3">
            @foreach ($section->items as $i => $item)
                <div x-data="{ open: {{ $i === 0 ? 'true' : 'false' }} }" x-reveal style="animation-delay: {{ $i * 60 }}ms" class="overflow-hidden rounded-xl border border-slate-100 bg-white">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="prose-cms px-5 pb-4 text-sm text-slate-600" style="display: none">{!! $item->bodyHtml() !!}</div>
                </div>
            @endforeach
        </div>
    </div>
