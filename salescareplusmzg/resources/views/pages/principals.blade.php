<x-layout title="Our Principals" description="Sales Care Plus MZG is the authorized distributor for leading pharmaceutical manufacturers, proudly representing trusted brands across Muzaffargarh and South Punjab.">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="reveal mx-auto max-w-2xl text-center">
            <h2 class="text-3xl font-bold text-navy-900 sm:text-4xl">
                Authorized Distributor for <span class="text-sky-500">{{ $principals->count() }}</span> Leading Companies
            </h2>
        ...
        <div class="reveal-stagger mt-14 grid grid-cols-1 gap-6 md:grid-cols-2">
            @foreach ($principals as $principal)
                <div class="flex gap-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <span class="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl bg-navy-900 text-lg font-bold text-sky-400">
                        {{ $principal->initials }}
                    </span>
                    <div>
                        <h3 class="text-lg font-bold text-navy-900">{{ $principal->name }}</h3>
                        <p class="mt-0.5 text-sm text-slate-500">{{ $principal->tagline }}</p>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <span class="inline-flex items-center gap-1 rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-semibold text-sky-700">
                        ...
                        <dl class="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4">
                            <div>
                                <dt class="text-lg font-bold text-navy-900">{{ $principal->years_partnership }}+</dt>
                                <dd class="text-[11px] text-slate-500">Years Partnership</dd>
                            </div>
                            <div>
                                <dt class="text-lg font-bold text-navy-900">{{ $principal->products_count }}+</dt>
                                <dd class="text-[11px] text-slate-500">Products</dd>
                            </div>
                            <div>
                                <dt class="text-lg font-bold text-navy-900">100%</dt>
    ...
    <section class="bg-slate-50 py-20">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="reveal mx-auto max-w-2xl text-center">
                <span class="text-sm font-semibold uppercase tracking-wide text-sky-600">Partnership</span>
                <h2 class="mt-3 text-3xl font-bold text-navy-900 sm:text-4xl">Why Partner With Us?</h2>
            ...
            <div class="reveal-stagger mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ([
                    ...
                    <div class="rounded-2xl border border-slate-100 bg-white p-6 text-center shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                        <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-sky-100 text-sky-600 transition-transform duration-300 hover:scale-110">
                            <x-icon :name="$reason['icon']" class="h-6 w-6" />
                        </span>
                        <h3 class="mt-4 font-semibold text-navy-900">{{ $reason['title'] }}</h3>
    ...
    <section class="bg-gradient-to-br from-navy-900 to-navy-950">
        <div class="reveal mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8">
            ...
            <a href="{{ route('contact') }}" class="mt-8 inline-flex items-center gap-2 rounded-full bg-sky-500 px-6 py-3.5 font-semibold text-white shadow-sm transition duration-300 hover:-translate-y-0.5 hover:bg-sky-400 hover:shadow-lg">
