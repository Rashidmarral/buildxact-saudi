<x-layout title="About Us" description="Building trust through quality pharmaceutical distribution — learn about Sales Care Plus MZG's story, mission and team in Muzaffargarh, Pakistan.">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="grid gap-12 lg:grid-cols-2 lg:items-start">
            <div class="reveal">
                <span class="text-sm font-semibold uppercase tracking-wide text-sky-600">Our Story</span>
                <h2 class="mt-3 text-3xl font-bold text-navy-900">Building Trust Through Quality Distribution</h2>
                ...
                <ul class="mt-8 space-y-4">
                    @foreach ([
                        ...
                        <li class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="font-semibold text-navy-900">{{ $point['title'] }}</p>
            ...
            <div class="reveal-stagger grid grid-cols-2 gap-4 rounded-3xl bg-slate-50 p-6">
                <div class="rounded-2xl bg-white p-6 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1">
                    <p class="text-3xl font-bold text-navy-900">{{ config('company.stats.years') }}+</p>
                    <p class="mt-1 text-sm text-slate-500">Years of Excellence</p>
                </div>
                <div class="rounded-2xl bg-white p-6 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1">
                    <p class="text-3xl font-bold text-navy-900">{{ config('company.stats.professionals') }}+</p>
                    <p class="mt-1 text-sm text-slate-500">Professionals</p>
                </div>
                <div class="rounded-2xl bg-white p-6 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1">
                    <p class="text-3xl font-bold text-navy-900">{{ $principalsCount }}</p>
                    <p class="mt-1 text-sm text-slate-500">Principal Companies</p>
                </div>
                <div class="rounded-2xl bg-white p-6 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1">
                    <p class="text-3xl font-bold text-navy-900">{{ config('company.stats.monthly_reach') }}</p>
            ...
            <div class="reveal-stagger mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 sm:px-6 md:grid-cols-4 lg:px-8">
                @foreach ($certifications as $certification)
                    <div class="flex items-center gap-3">
                        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-sky-600 shadow-sm">
                            <x-icon :name="$certification->icon" class="h-5 w-5" />
                        </span>
                        <span class="text-sm font-medium text-navy-900">{{ $certification->title }}</span>
                    </div>
                @endforeach
            </div>
        </section>
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="reveal-stagger grid gap-6 md:grid-cols-3">
            <div class="rounded-2xl border border-slate-100 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-navy-900 text-sky-400">
                    <x-icon name="badge-check" class="h-6 w-6" />
                </span>
                <h3 class="mt-5 text-xl font-bold text-navy-900">Our Mission</h3>
            ...
            <div class="rounded-2xl border border-slate-100 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                ...
                <h3 class="mt-5 text-xl font-bold text-navy-900">Our Vision</h3>
                <p class="mt-3 leading-relaxed text-slate-600">
            ...
            <div class="rounded-2xl border border-slate-100 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                ...
                <h3 class="mt-5 text-xl font-bold text-navy-900">Our Values</h3>
                <p class="mt-3 leading-relaxed text-slate-600">
        ...
        <section class="bg-navy-950 py-20 text-white">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="reveal mx-auto max-w-2xl text-center">
                    <span class="text-sm font-semibold uppercase tracking-wide text-sky-400">Leadership</span>
                ...
                <div class="reveal-stagger mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($team as $member)
                        <div class="rounded-2xl border border-navy-800 bg-navy-900/60 p-6 text-center transition duration-300 hover:-translate-y-1 hover:border-sky-500">
                            <span class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-sky-500 text-lg font-bold text-white transition-transform duration-300 hover:scale-110">
                                {{ $member->initials }}
                            </span>
                            <h3 class="mt-4 font-semibold">{{ $member->name }}</h3>
                            <p class="text-sm text-sky-400">{{ $member->designation }}</p>
                            <p class="mt-3 text-xs leading-relaxed text-navy-200">{{ $member->bio }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
