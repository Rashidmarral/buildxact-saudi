<x-layout title="Our Services" description="From warehousing to last-mile delivery, discover the distribution services Sales Care Plus MZG provides to pharmacies, clinics and hospitals in Muzaffargarh.">
    ...
    <section class="bg-teal-pattern py-16 sm:py-20">
        <div class="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            ...
            <h1 class="hero-fade-in hero-fade-in-delay-1 mt-6 text-4xl font-bold tracking-tight text-white sm:text-5xl">End-to-End Distribution Services</h1>
    ...
    <section class="mx-auto max-w-7xl px-4 pt-16 sm:px-6 lg:px-8">
        <div class="grid gap-10 lg:grid-cols-2 lg:items-center">
            ...
            <div class="reveal">
                <span class="text-sm font-semibold uppercase tracking-wide text-coral-600">Reliable, Every Route</span>
                <h2 class="mt-3 text-3xl font-bold text-teal-900">Built for the Realities of Local Distribution</h2>
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="reveal-stagger grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            @foreach ([
                ...
                ['icon' => 'headset', 'title' => 'Dedicated Account Support', 'text' => 'A named representative for every partner pharmacy — for order queries, product information and account support.'],
            ] as $service)
                <div class="rounded-2xl border border-slate-100 p-7 shadow-sm transition duration-300 hover:-translate-y-1.5 hover:shadow-lg">
                    ...
                    <p class="mt-3 leading-relaxed text-slate-600">{{ $service['text'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
            ...
            <ol class="reveal-stagger mt-14 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ([
                    ...
                    <li class="relative rounded-2xl bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                        <span class="text-3xl font-bold text-teal-200">{{ $item['step'] }}</span>
                        <h3 class="mt-3 font-semibold text-slate-800">{{ $item['title'] }}</h3>
