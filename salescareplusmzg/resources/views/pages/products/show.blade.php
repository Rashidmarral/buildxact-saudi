<x-layout :title="$product->name" :description="$product->name.' — '.$product->generic_name.'. Distributed by Sales Care Plus MZG across Muzaffargarh and South Punjab.'">
    ...
    <section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div class="grid gap-12 lg:grid-cols-2">
            <div class="flex aspect-square items-center justify-center rounded-3xl bg-gradient-to-br from-leaf-100 to-leaf-50">
                <x-icon name="pill" class="h-28 w-28 text-leaf-500" />
            </div>
            ...
            <div>
                ...
                <div class="mt-8 flex flex-wrap gap-4">
                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 rounded-full bg-leaf-600 px-6 py-3.5 font-semibold text-white shadow-sm transition hover:bg-leaf-700">
                        Request Pricing &amp; Availability <x-icon name="arrow-right" class="h-4 w-4" />
                    </a>
                    <a href="tel:{{ config('company.phone') }}" class="inline-flex items-center gap-2 rounded-full border border-leaf-300 px-6 py-3.5 font-semibold text-leaf-700 transition hover:bg-leaf-50">
            ...
            <div class="mt-20">
                ...
                <div class="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($related as $item)
                        <a href="{{ route('products.show', $item) }}" class="group rounded-2xl border border-stone-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
