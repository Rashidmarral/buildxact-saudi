        ...
        <div class="mx-auto grid max-w-7xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-24">
            <div>
                <span class="inline-flex items-center gap-2 rounded-full bg-leaf-100 px-4 py-1.5 text-sm font-medium text-leaf-700">
                    <x-icon name="leaf" class="h-4 w-4" /> Rooted in Muzaffargarh since {{ config('company.founded_year') }}
                </span>
                <h1 class="mt-6 text-balance text-4xl font-bold tracking-tight text-leaf-950 sm:text-5xl lg:text-6xl">
                ...
                <div class="mt-8 flex flex-wrap gap-4">
                    <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 rounded-full bg-leaf-600 px-6 py-3.5 font-semibold text-white shadow-sm transition hover:bg-leaf-700">
                        Explore Our Products <x-icon name="arrow-right" class="h-4 w-4" />
                    </a>
                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 rounded-full border border-leaf-300 bg-white px-6 py-3.5 font-semibold text-leaf-700 transition hover:bg-leaf-50">
                        Get in Touch
                    </a>
                </div>
                <dl class="mt-12 grid grid-cols-3 gap-6 border-t border-leaf-200 pt-8">
                    <div>
                        <dt class="text-2xl font-bold text-leaf-800 sm:text-3xl">10+</dt>
                        <dd class="mt-1 text-sm text-stone-500">Years of Service</dd>
                    </div>
                    <div>
                        <dt class="text-2xl font-bold text-leaf-800 sm:text-3xl">300+</dt>
                        <dd class="mt-1 text-sm text-stone-500">Partner Pharmacies</dd>
                    </div>
                    <div>
                        <dt class="text-2xl font-bold text-leaf-800 sm:text-3xl">8</dt>
            ...
            <div class="relative">
                <div class="aspect-[4/5] w-full overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-leaf-600 via-leaf-700 to-leaf-900 shadow-2xl">
                    <div class="flex h-full w-full flex-col items-center justify-center gap-6 p-10 text-center text-leaf-50">
                        <x-icon name="leaf" class="h-20 w-20 text-leaf-300" />
                ...
                <div class="absolute -bottom-6 -left-6 hidden rounded-2xl bg-white p-5 shadow-xl sm:block">
    ...
    <section class="border-y border-leaf-100 bg-white">
        <div class="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-8 sm:px-6 md:grid-cols-4 lg:px-8">
            @foreach ([
                ...
                <div class="flex items-center gap-3">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-leaf-50 text-leaf-700">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="grid gap-12 lg:grid-cols-2 lg:items-center">
            <div class="order-2 grid grid-cols-2 gap-4 lg:order-1">
                <div class="col-span-2 flex h-48 items-center justify-center rounded-2xl bg-leaf-100 text-leaf-600 sm:h-64">
                    <x-icon name="warehouse" class="h-16 w-16" />
                </div>
                <div class="flex h-32 items-center justify-center rounded-2xl bg-sand-100 text-sand-700 sm:h-40">
                    <x-icon name="thermometer" class="h-10 w-10" />
                </div>
                <div class="flex h-32 items-center justify-center rounded-2xl bg-leaf-600 text-white sm:h-40">
            ...
            <div class="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($categories as $category)
                    <a href="{{ route('products.index', ['category' => $category->slug]) }}"
                       class="group rounded-2xl border border-leaf-100 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                        <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-leaf-100 text-leaf-700 transition group-hover:bg-leaf-600 group-hover:text-white">
            ...
            <div class="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($featuredProducts as $product)
                    <a href="{{ route('products.show', $product) }}" class="group rounded-2xl border border-stone-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                        <div class="flex h-28 items-center justify-center rounded-xl bg-leaf-50 text-leaf-500">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        ...
        <div class="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
            @foreach ($testimonials as $testimonial)
                <figure class="flex flex-col rounded-2xl border border-leaf-100 bg-leaf-50/50 p-7">
    ...
    <section class="bg-sand-500">
        <div class="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8">
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-4">
                <a href="{{ route('contact') }}" class="rounded-full bg-white px-6 py-3.5 font-semibold text-sand-700 shadow-sm transition hover:bg-sand-50">
                    Request a Callback
                </a>
                <a href="tel:{{ config('company.phone') }}" class="rounded-full border border-white/70 px-6 py-3.5 font-semibold text-white transition hover:bg-white/10">
