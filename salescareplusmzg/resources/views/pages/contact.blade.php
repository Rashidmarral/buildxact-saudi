<x-layout title="Contact Us" description="Get in touch with Sales Care Plus MZG for medicine orders, pharmacy account setup, or general enquiries. Muzaffargarh, Punjab, Pakistan.">
    ...
    <section class="bg-leaf-pattern bg-leaf-50 py-16 sm:py-20">
        <div class="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            ...
            <h1 class="mt-6 text-4xl font-bold tracking-tight text-leaf-950 sm:text-5xl">We'd Love to Hear From You</h1>
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="grid gap-12 lg:grid-cols-5">
            ...
            <div class="lg:col-span-2">
                <div class="space-y-6">
                    <div class="flex items-start gap-4 rounded-2xl border border-stone-100 p-6">
            ...
            <div class="lg:col-span-3">
                <div class="rounded-3xl border border-stone-100 p-8 shadow-sm">
                    @if (session('status'))
                        <div class="mb-6 flex items-center gap-2 rounded-xl bg-leaf-50 px-4 py-3 text-sm font-medium text-leaf-700">
                    ...
                    <form method="POST" action="{{ route('contact.store') }}" class="space-y-5">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                    class="mt-1.5 w-full rounded-lg border border-stone-200 px-4 py-2.5 text-sm focus:border-leaf-500 focus:outline-none focus:ring-1 focus:ring-leaf-500">
                                @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="message" id="message" rows="5" required
                                class="mt-1.5 w-full rounded-lg border border-stone-200 px-4 py-2.5 text-sm focus:border-leaf-500 focus:outline-none focus:ring-1 focus:ring-leaf-500">{{ old('message') }}</textarea>
                            @error('message')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <button type="submit" class="inline-flex items-center gap-2 rounded-full bg-leaf-600 px-6 py-3.5 font-semibold text-white shadow-sm transition hover:bg-leaf-700">
